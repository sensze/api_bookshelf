...
const addBookHandler = (request, h) => {
  ...
  books.push(newBook); // push buku ke array books

  const isSuccess = books.filter((note) => note.id === id).length > 0; // cek jika newBook pushed
  ...
  // Server gagal memasukkan buku (generic error).
  const response = h
    .response({
      status: 'fail',
